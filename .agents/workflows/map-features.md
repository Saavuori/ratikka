...
* Each vehicle is drawn as a small stack of layers, all reading the `trams` GeoJSON source (properties written per-frame in the tick loop: `hdg`, `mode`, `spd`, `acc`, `speedNorm`, `doorsOpen`, `stopped`):
  * `trams-circles` — the **motion aura** (kept under this id because many other layers use it as their `beforeId` anchor). A blurred circle that grows with `speedNorm` and is tinted by `acc`: green accelerating, red braking, mode-neutral cruising; fades to nothing at a standstill. `speedNorm` is `clamp(spd / 8, 0, 1)` and the opacity ramp snaps up as soon as a vehicle moves (≈0.45 → 0.62), with a tight `circle-blur` of 0.4, so a moving vehicle reads at a glance rather than being an easy-to-miss faint glow.
  * `trams-stopped` — a **subtle, borderless coral (`#e17055`) glow** marking a **stopped** vehicle (`stopped` = `doorsOpen || spd === 0`). Because the motion aura fades to nothing at a standstill, this is the positive "I'm stopped" cue for a halted tram (red light, traffic, terminus). It is a soft, high-`circle-blur` (0.85) disc with **no stroke** — deliberately distinct from the crisp gold selection ring so the two are never confused — and collapses to radius/opacity 0 once moving. There is intentionally **no** doors-open pulse layer: the `-open` body art (amber door gaps) is the sole boarding cue.
