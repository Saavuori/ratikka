...
### Fixed
- **A route no longer appears twice, once either side of its own street.** The fan-out shipped in v0.47.2 offset each highlighted line into its own slot — but `line-offset` is signed relative to a path's *direction of travel*, and `/api/v1/route/{n}` returns one polyline per pattern. A route's outbound and inbound patterns are near-reverses of each other, so they were pushed to opposite sides and the single route read as two ribbons — sometimes three, because a route ships more than two patterns and a short turn is not the reverse of anything, it is a subset. Two things fix it. Path direction is canonicalised before the offset is applied, decided on the path's *dominant* axis rather than on longitude alone, which for a north/south route with a little sideways drift was close to a coin flip. Then a line's paths are reduced to the ground they actually cover — quantised to a grid, longest first, keeping only what visits somewhere nothing kept so far has been. Comparing coverage rather than direction is what makes this robust: a reversed duplicate contributes nothing new whichever way it runs, a short turn is a subset, and a genuine branch survives. The check is tolerant of a neighbouring cell, so the two directions of a bus route — twenty-odd metres apart on opposite sides of the street — collapse into one ribbon rather than half-scoring as new ground.
