...
export function updateStopVisibility(
  ...
  const { modes, lineFilters, selectedVehicleId, vehicles, routeGeometries, selectedStopId } = view;

  // Build the list of active lines we want to show stops for
  const activeRoutes = [...lineFilters];
  const selectedTram = selectedVehicleId ? vehicles[selectedVehicleId] : null;
