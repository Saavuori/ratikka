...
A premium, high-performance web application mapping **all active Helsinki trams, buses, metro, commuter trains, ferries, and City Bike stations** in real-time. Built with stunning glassmorphism aesthetics, fluid 60fps telemetry interpolation, and immersive interactive modes.
...
* **Real-time 60fps Vehicle Interpolation**: Live MQTT vehicle coordinate updates for **trams, buses, metro, commuter trains and ferries** are mathematically interpolated (lerp) for buttery-smooth vehicle movement.
...
* **Ride Along — the map finds the vehicle you are in**: tap **I'm on board** and your own position is matched against the live feed to work out which vehicle is carrying you; the app then selects it, opens its route and stop list, and locks the camera to it, so the map is about the run you are actually on. One fix cannot tell a ride from a tram passing the pavement you are standing on, so the evidence is gathered over a rolling window — how long a vehicle has stayed within reach, how close it has been, and how much ground the two of you have covered *together* — and how far away still counts is the vehicle's own length plus the phone's reported error plus the distance it has run since its last report. A vehicle is claimed outright only after a hundred and fifty metres of shared travel; on weaker evidence it is offered as a question. Two vehicles on different journeys that fit equally well are reported as ambiguous rather than guessed between, and stepping off ends the ride and releases the camera. Nothing starts on its own: the watch begins on a tap, and resumes unprompted only where the permission is already granted.
* **Tells You When To Get Off**: a selected journey watches the leg you are riding and counts your stop down at the bottom of the map — gold two stops out, coral and a buzz when the vehicle's next stop is yours, **Get off here** when the doors open at it, and the same two moments on the lock screen once notifications are allowed. The count comes from the vehicle running your leg, looked up in the leg's own stop list; where no vehicle can be matched unambiguously it falls back to the arrival prediction and says so, since "the tram says your stop is next" and "the timetable thinks you arrive in ninety seconds" are different claims. A stop already behind the vehicle is reported as passed rather than counted down to.
...
* **Who Is Asking This Crossing**: click a traffic light and the exchange reads from the junction's side — every vehicle currently negotiating with it, grouped into priority granted, still requesting, refused, and reached-it-but-did-not-ask. Each row carries the line in its own colour, the distance still to run, whether the vehicle is rolling or standing, what it asked for and on which attempt, and how long ago; picking one opens that vehicle's own panel. The selected junction takes the selection gold on its ring — never on its lenses, which go on saying what was asked.
* **Bunched Trams and the Gaps They Leave**: two trams of the same line nose to tail, then nothing for a quarter of an hour, is the commonest way a line fails its riders, and no single vehicle's delay shows it — only the pair does. The backend times every vehicle out of every stop (the feed names the stop each vehicle is heading for, so the moment that changes is the moment it left), and each position carries how far it is running behind the vehicle ahead: the gap a rider at that stop actually waited. What the gap *should* be comes from the same passages — each vehicle's departure minus its reported delay is when it was due — and a line's headway is the median across its pairs, so a cancelled trip shows as the hole it leaves rather than as a new timetable. Bunched pairs (within 30% of the headway) are tied together on the map with a coral bar; a gap (1.8× the headway, and three minutes over it) runs in amber dashes along the rails between the two vehicles on any line you pick out. The **Lines** panel lists the worst of both with a dot on each affected line, and a vehicle's own panel shows the vehicles either side with the spacing to each drawn to scale. Between stops a gap is known only to be *at least* so long, and says so; a tram laying over at its terminus is never called bunched on a bound alone.
...
* **The Ferry, and How Full It Is**: The fifth mode is the Suomenlinna crossing, and it is the only one whose HFP `occu` field is a measurement rather than a placeholder — every road and rail vehicle sends a constant `0`, the boats count passengers. So the ferry is the one vehicle on the map that shows its own load. Its marker is a vessel rather than a carriage — raked bow, wide beam, deckhouse inset between open decks — and the deckhouse is a saloon that fills from the stern forward in a colour that runs green through amber to red; the 3D body carries the same gauge along its saloon roof, where a pitched camera looks straight down on it; the telemetry panel states it in words and a meter. A vessel nobody has counted gets an empty **grey** gauge, never a green one: "no report" is not "nobody aboard".
* **Detailed 3D Vehicles**: Distinct tram, bus, metro, commuter-train and ferry bodies have articulated sections, windows, running gear and roof equipment — the ferry instead a hull, a saloon, a bridge and a funnel, with a wake astern while it is making way and no wheels at all. A vehicle running on rails **bends at its joints**: each rigid section is placed at its own point on the track at the bearing the rails have there, with the gangway stretching between them, so a 27 m tram takes a street corner instead of ploughing its nose through the building outside the curve. Models fade in between zoom 13 and 14 (previously 15–16), retaining real-world dimensions. The vehicle button in the map-view controls enables **Always show 3D vehicles**, including on the flat map, independently of map tilt; this preference persists. Below zoom 13, icons remain visible for readability.
...
* **Animated 2D Vehicle Schematic**: Mode-accurate vector layouts (3 door pairs for trams, 2 for buses, a hull in the water for the ferry) that open doors live from the `drst` flag, blink boarding indicators, and spin wheels at a rate proportional to velocity. See [docs/ICONS_AND_ANIMATIONS.md](docs/ICONS_AND_ANIMATIONS.md) for a full catalogue of every icon and animation with source examples.
...
* **MQTT Ingestion**: Subscribes to `/hfp/v2/journey/ongoing/vp/tram/#` on `tls://mqtt.hsl.fi:8883`, plus the `bus`, `metro`, `train` and `ferry` topics on demand — each only while a connected client has that mode switched on.
